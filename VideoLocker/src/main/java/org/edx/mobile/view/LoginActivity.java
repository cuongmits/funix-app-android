...
public class LoginActivity extends BaseFragmentActivity implements SocialLoginDelegate.MobileLoginCallback {
    ...
    public void onUserLoginSuccess(ProfileModel profile) throws LoginException {
        ...
        String em = email_et.getText().toString();
        String pa = password_et.getText().toString();
        String au = "Basic " + Base64.encodeToString((email_et.getText().toString() + ":" + password_et.getText().toString()).getBytes(), Base64.NO_WRAP);

        //KEON
        pref.put(PrefManager.Key.AUTH_REST, "Basic " + Base64.encodeToString((email_et.getText().toString() + ":" + password_et.getText().toString()).getBytes(), Base64.NO_WRAP));
        pref.put(PrefManager.Key.AUTH_REST_CHECKER, email_et.getText().toString() + ":" + password_et.getText().toString());
